...
void parse_staking_incoming_data(transaction_version_t version, uint8_t *data, uint16_t data_length, uint8_t *sender,
    ...
    if (version == TRANSACTION_VERSION_LEGACY) {
        PRINTF("Staking is not supported for legacy transactions");
        THROW(0x6a80);
    }
    ...
    switch (out->type) {
        case CREATE_STAKER:
        case UPDATE_STAKER: {
            bool hasDelegation = readBool(&data, &data_length);
            if (hasDelegation) {
                uint8_t *delegation_address_pointer = readSubBuffer(20, &data, &data_length);
            ...
            if (out->type == UPDATE_STAKER) {
                bool reactivate_all_stake = readBool(&data, &data_length);
        ...
        case RETIRE_STAKE: {
            uint64_t amount = readUInt64(&data, &data_length);
    ...
    if (data_length != 0) {
        PRINTF("Incoming staking data too long");
        THROW(0x6700);
    }
...
staking_outgoing_data_type_t parse_staking_outgoing_data(transaction_version_t version, uint8_t *sender_data,
    ...
    staking_outgoing_data_type_t type = readUInt8(&sender_data, &sender_data_length);
    if (type != DELETE_VALIDATOR && type != REMOVE_STAKE) {
        PRINTF("Invalid outgoing staking type");
        THROW(0x6a80);
    }

    if (sender_data_length != 0) {
        PRINTF("Outgoing staking data too long");
        THROW(0x6700);
    }
